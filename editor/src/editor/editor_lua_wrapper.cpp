...
ToolbarWrapper::ToolbarWrapper(std::string const& name, sol::function callback) {
    ...
    auto it = editor::renderer::add_toolbar(
        name, [callback](Toolbar& t) { callback(ToolbarWrapper(t.id)); });
    id = it.id;
}

void ToolbarWrapper::lua_destroy() { editor::renderer::remove_toolbar(id); }
...
void WindowWrapper::lua_destroy() { editor::renderer::remove_window(id); }
