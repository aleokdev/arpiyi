...
u32 Tileset::get_id_auto(u32 x_index, u32 surroundings) const {
    ...
    // Delete corners if there's a side next to them (since those cases are not covered by the tile table)
    ...
    if(it == tile_table.end()) assert(false);
    return x_index + get_size_in_tiles(tileset_manager::get_tile_size()).x * static_cast<u8>(std::distance(tile_table.begin(), it));
}

u32 Tileset::get_surroundings_from_auto_id(u32 id) const {
    const auto tileset_size_in_tiles{get_size_in_tiles()};
    const u32 auto_tiletable_index = (id - (id % tileset_size_in_tiles.x)) / tileset_size_in_tiles.x;
