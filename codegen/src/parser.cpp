...
std::vector<AttributedEntity> parse_cpp_file(fs::path const& path) {
    ...
    std::string_view file_view = file_data;
    ...
    while (!file_view.empty()) {
        ...
        } else if (type_read == EntityType::struct_t && next_token_is_attribute(file_view)) {
            attribute_queue.emplace_back(parse_attribute(file_view));
        } else if(!std::isspace(file_view[0]) && type_read == EntityType::struct_t) {
